...
RSpec.describe "crossed wires" do
  ...
  context "part two" do
    specify "example one" do
      path1 = Path.from_string( "R75,D30,R83,U83,L12,D49,R71,U7,L72" )
      path2 = Path.from_string( "U62,R66,U55,R34,D71,R55,D58,R83" )
      expect( CrossedWires.steps_to_fastest_intersection(path1, path2) ).to eq( 610 )
    end

    specify "example two" do
      path1 = Path.from_string( "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51" )
      path2 = Path.from_string( "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" )
      expect( CrossedWires.steps_to_fastest_intersection(path1, path2) ).to eq( 410 )
    end

    xspecify "solution" do
      expect( CrossedWires.steps_to_fastest_intersection(path_one, path_two) ).to eq( 107647 )
    end
  end
end
