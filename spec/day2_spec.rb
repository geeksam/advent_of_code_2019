...
RSpec.describe 'a ship computer' do
  specify "1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2)" do
    expect( ShipComputer.execute("1,0,0,0,99") ).to eq( "2,0,0,0,99" )
  end

  specify "2,3,0,3,99 becomes 2,3,0,6,99 (3 * 2 = 6)" do
    expect( ShipComputer.execute("2,3,0,3,99") ).to eq( "2,3,0,6,99" )
  end

  specify "2,4,4,5,99,0 becomes 2,4,4,5,99,9801 (99 * 99 = 9801)" do
    expect( ShipComputer.execute("2,4,4,5,99,0") ).to eq( "2,4,4,5,99,9801" )
  end

  specify "1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99" do
    expect( ShipComputer.execute("1,1,1,4,99,5,6,0,99") ).to eq( "30,1,1,4,2,5,6,0,99" )
  end
